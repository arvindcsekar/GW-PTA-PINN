...
t_pulsar_specific_years = np.linspace(-(delta_t), 0, 1000) / u.yr.to(u.s) 
...
plt.plot((t_pulsar_phys / u.yr.to(u.s)), omega_tensor_pulsar.detach().numpy(), label="Pulsar", color="red", linestyle="--")
plt.show()
plt.plot((t_earth_phys/ u.yr.to(u.s)), omega_tensor_earth.detach().numpy(), label="Earth", color="blue", linestyle="-")
plt.show()
...
plt.title("Evolution of SMBHB Orbital Frequency with Time at 0.5 χ1, 0.5 χ2, and 1e9 m1, m2")
...
def compute_residual_loss_phi(model2, t_batch, model):
    ...
    dphi_dt_phys = dphi_dtau / t_range
