...
def compute_residual_loss(model, num_points):
    ...
    chi_1_res = torch.rand(num_points, 1, dtype=torch.float64) * 2.0 - 1.0
    chi_2_res = torch.rand(num_points, 1, dtype=torch.float64) * 2.0 - 1.0

    m1_phys = (torch.rand(num_points, 1, dtype=torch.float64) * (1e10 - 1e9) + 1e9) * M_sun_SI
    m2_phys = (torch.rand(num_points, 1, dtype=torch.float64) * (1e10 - 1e9) + 1e9) * M_sun_SI
...
sample_chi2 = torch.full_like(t_tensor, 0, dtype=torch.float64)
...
plt.legend(["2PN: Spin","Newtonian", "ω-PINN"])
plt.title("Evolution of SMBHB Orbital Frequency with Time at 0.5 χ1, χ2")
